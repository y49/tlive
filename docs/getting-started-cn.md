...
2. 权限提示（或 `AskUserQuestion`）在终端等你时，tlive 只提醒**这台机器**——
   桌面通知 + dashboard 上一张只读的 *waiting-approval* 卡片，因为只有坐在
   机器前的人才能处理它。IM 对这类只有终端能答的对话保持安静（手机够不到
   终端），每个会话只有第一次例外：届时会收到一张卡解释原因，并带一键切到
   `full` 的按钮。
