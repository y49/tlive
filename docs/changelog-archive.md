# Changelog archive (pre-2.0.0)

**Frozen.** This is the hand-written changelog kept up to the 2.0.0 beta. It
holds two kinds of writing that a changelog shouldn't mix: per-version "what
changed" lists, and long narrative on *why* v2 replaced v1. From 2.0.0-beta.0
on, [`CHANGELOG.md`](../CHANGELOG.md) is generated by release-please from
Conventional Commits and nothing is hand-written into it; design narrative
belongs in `docs/` and in GitHub Release bodies.

Nothing below is maintained. The two v2 sections describe the work that ships
*in* 2.0.0-beta.0 — they predate the first automated release, so the generated
changelog covers the same ground more tersely.

## v2 — web terminal + dashboard + IM interaction layer
