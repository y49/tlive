// src/cli/stop-daemon.ts — `tlive stop-daemon`
//
// Requests an orderly shutdown via IPC. Falls back to SIGTERM on the PID
// file when the socket is unreachable. Never force-kills; operators can use
// `kill -9` if needed.
...
export async function stopDaemonCommand(): Promise<void> {
...
if (process.argv[1]?.endsWith('tlive-stop-daemon.mjs')) {
